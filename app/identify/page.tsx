  ...
  return (
    <main className="relative flex min-h-screen flex-col items-center overflow-hidden bg-[#1a3a2a] p-4">
      ...
      <div className="relative z-10 w-full max-w-md pt-6">
        <Link href="/" className="text-xl text-white/70 hover:text-white">
          ← Back
        </Link>
          ...
          <div className="mt-6 flex flex-col gap-4">
            {preview ? (
              <div className="relative h-80 w-full overflow-hidden rounded-2xl">
                ...
                <button
                  ...
                  className="absolute top-2 right-2 rounded-full bg-black/50 px-2 py-0.5 text-md text-white"
                >
                  Remove
                </button>
              </div>
              ...
              <div className="flex gap-3">
                <button
                  type="button"
                  onClick={startCamera}
                  className="flex flex-1 flex-col items-center justify-center gap-2 rounded-2xl border-2 border-white/30 bg-[#1a3a2a] py-8 text-white transition-colors hover:bg-[#2d6a4f]"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-20 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M6.827 6.175A2.31 2.31 0 015.186 7.23c-.38.054-.757.112-1.134.175C2.999 7.58 2.25 8.507 2.25 9.574V18a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 18V9.574c0-1.067-.75-1.994-1.802-2.169a47.865 47.865 0 00-1.134-.175 2.31 2.31 0 01-1.64-1.055l-.822-1.316a2.192 2.192 0 00-1.736-1.039 48.774 48.774 0 00-5.232 0 2.192 2.192 0 00-1.736 1.039l-.821 1.316z" />
                    <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 12.75a4.5 4.5 0 11-9 0 4.5 4.5 0 019 0zM18.75 10.5h.008v.008h-.008V10.5z" />
                  </svg>
                  <span className="text-xl font-medium">Use Camera</span>
                </button>
            ...
            {selectedFile && stage !== 'loading' && (
              <button
                ...
                className="w-full rounded-xl bg-[#1a3a2a] py-3 text-xl font-medium text-white transition-colors hover:bg-[#2d6a4f]"
              >
                Identify Plant
              </button>
            )}
            ...
            {stage === 'loading' && (
              <div className="flex w-full items-center justify-center gap-2 rounded-xl bg-white/10 py-3 text-md text-white/70">
        ...
        {stage === 'result' && result && topSuggestion && (
          <div className="mt-6 flex flex-col gap-4">
            ...
            <div className="flex gap-3">
              <button
                onClick={handleReset}
                className="flex-1 rounded-xl border border-white/30 bg-white/10 py-3 text-md text-white transition-colors hover:bg-white/20"
              >
                Try another
              </button>
              <button
                onClick={() => setStage('flashcard')}
                className="flex-1 rounded-xl bg-[#1a3a2a] py-3 text-lg font-medium text-white transition-colors hover:bg-[#2d6a4f]"
              >
                Create Flashcard
              </button>
            </div>
        ...
        {stage === 'flashcard' && result && topSuggestion && (
          <div className="mt-6 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1">
              <label htmlFor="note" className="text-md font-medium text-white">
                Personal note <span className="font-normal text-white/50">(optional)</span>
              </label>
              <textarea
                id="note"
                value={personalNote}
            ...
            {flashcardError && (
              <p className="rounded-xl bg-red-50 px-3 py-2 text-md text-red-600">{flashcardError}</p>
            )}

            <button
              onClick={handleSaveFlashcard}
              disabled={savingFlashcard}
              className="w-full rounded-xl bg-[#1a3a2a] py-3 text-lg font-medium text-white transition-colors hover:bg-[#2d6a4f] disabled:opacity-50"
            >
        ...
        {stage === 'error' && (
          <div className="mt-6 flex flex-col gap-4">
            ...
            <button
              onClick={handleReset}
              className="w-full rounded-xl border border-white/30 bg-white/10 py-3 text-md text-white transition-colors hover:bg-white/20"
            >
              Try again
            </button>
          </div>
